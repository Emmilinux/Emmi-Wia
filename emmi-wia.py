...
"""
 Emmi WIA 1.3
 Copyright (C) 2010
 Baseado em WIA de Palumbo Roberto <palumborobertomail@gmail.com>
 Author: Mario Colque <mario@emmi.org.ar>
 Editado por Sidnei Weber <sidnei@emmilinux.com.br>
 Emmi Team! - www.emmilinux.com.br
...
a = int(commands.getoutput('ps -A | grep emmi-wia | wc -l'))
if a > 1:
	exit(0)

# i18n
gettext.install('emmi-wia', '/usr/share/emmi/locale')

# Inicia Notificaciones
if not pynotify.init ('EMMI-WIA-Notify'):
	exit(1)
	...
	def searchIn(self, path):
		...
		for sele in selez:
			...
			if frase[0] == 'I':
				...
				catname = ''
				image = '/usr/lib/emmi/emmi-wia/wia-add.png'
				f = open(sel,'r')
			...
			if frase[0] == 'E':
				notify(_('Apps deleted found'), _('Emmi confirm that application was deleted'), '/usr/lib/emmi/emmi-wia/wia-remove.png')
			os.system('aplay /usr/lib/emmi/emmi-wia/wia.wav &')
